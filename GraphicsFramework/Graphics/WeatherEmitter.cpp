...
#define RAND1() (rand()%202)
#define RAND2() (rand()%101)

/*
Constructor, which sets everything to some 'sensible' defaults.
*/
WeatherEmitter::WeatherEmitter(void)	{
	particleRate		= 100.0f;
	particleLifetime	= 50.0f;
	particleSize		= 0.0f;
	particleVariance	= 0.0f;
	nextParticleTime	= 0.0f;
	particleSpeed		= 0.0f;
	numLaunchParticles	= 0;
	largestSize			= (unsigned int)0.5;
...
*/
WeatherEmitter::~WeatherEmitter(void)	{
	for(std::vector<WeatherParticle *>::iterator i = particles.begin(); i != particles.end(); ++i) {
		delete (*i);
	}
	for(std::vector<WeatherParticle *>::iterator i = freeList.begin(); i != freeList.end(); ++i) {
		delete (*i);
	}
}
...
*/
void WeatherEmitter::Update(float msec, Vector3 cam)	{
	...
	*/
	...
	for(std::vector<WeatherParticle *>::iterator i = particles.begin(); i != particles.end();/*No I++ here!!! */) {
		WeatherParticle *p = (*i);
...
*/
WeatherParticle* WeatherEmitter::GetFreeParticle()	{
	WeatherParticle * p = NULL;
	...
	if(freeList.size() > 0) {
		...
		p = new WeatherParticle();
	}

	posDir(weatherType, p);


	return p;	//return the new particle
}
void WeatherEmitter::posDir(int wt, WeatherParticle* p)
{
	//Now we have to reset its values - if it was popped off the
	//free list, it'll still have the values of its 'previous life'

	// depending on 'weatherType' different directions and positions are set
	// for each weather condition
	if(wt == 1)
	{
		p->colour		= Vector4(colour2);
		p->direction	= initialDirection;
		p->direction.x += ((RAND()-RAND()));
		p->direction.y = (-1);
		p->direction.z += ((RAND()-RAND()));

		//p->direction.x = -RAND();
		//p->direction.z = -RAND();
		//p->direction.Normalise();	//Keep its direction normalised!

		p->position.y = camera.y + 100 + (RAND());//camera.y;
		p->position.x = camera.x + (RAND1()-RAND1())*2.5f;
		p->position.z = camera.z + (RAND1()-RAND1())*2.5f;
	}
	...
	else if(wt == 2)
	{
		...
		p->position.y = camera.y + 10 + (RAND()*10);//camera.y;
		p->position.x = camera.x + (RAND2()-RAND2());
		p->position.z = camera.z + (RAND2()-RAND2());
	}
...
*/
void	WeatherEmitter::ResizeArrays() {
