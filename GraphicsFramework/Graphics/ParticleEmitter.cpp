...
ParticleEmitter::ParticleEmitter(void)	{
	...
	*/
	texture = SOIL_load_OGL_texture("../Textures/Particle.bmp",
...
ParticleEmitter::ParticleEmitter(int i)
{
	particleRate		= 1.0f;
...
ParticleEmitter::~ParticleEmitter(void)	{
	...
	for(vector<Particle *>::iterator i = freeList.begin(); i != freeList.end(); ++i) {
		delete (*i);
	}
}
...
void ParticleEmitter::Update(float msec)	{
	...
	*/
	...
	for(vector<Particle *>::iterator i = particles.begin(); i != particles.end();/*No I++ here!!! */) {
		...
		if(explosion)
		{
			if(rand2 == 0) p->colour.x +=  0.0075f;
			else if(rand2 == 1) p->colour.y += 0.0075f;
			else p->colour.z +=  0.0075f;
		}
...
Particle* ParticleEmitter::GetFreeParticle()	{
	...
	if(!explosion)
	{
		p->colour		= Vector4(0.9f,0.9f,1.0f,1.0f);
	...
	{
		if(rand1 == 0) p->colour = Vector4(0.95f, 0.0f, 0.0f,1.0f);
		else if(rand1 == 1) p->colour = Vector4(0.0f, 0.95f, 0.0f,1.0f);
		else p->colour = Vector4(0.0f, 0.0f, 0.95f, 1.0f);

		p->direction.x += RAND()-RAND(); // * particleVariance);
		p->direction.y += (RAND()-RAND()) / 250; //) * particleVariance);
		p->direction.z += RAND()-RAND(); //) * particleVariance);
