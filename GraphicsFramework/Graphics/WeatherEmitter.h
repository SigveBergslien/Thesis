...
class WeatherEmitter : public Mesh	{
public:
	WeatherEmitter(void);
	~WeatherEmitter(void);
	...
	*/
	void Update(float msec, Vector3 cam);
	...
	*/
	...
	void SetWeatherColour(Vector4 col)		{colour2 = col;}
	void SetWeatherType(int wt)				{weatherType = wt;}

	void posDir(int wt, WeatherParticle* p);
...
protected:
	...
	*/
	WeatherParticle* GetFreeParticle();
	...
	*/
	...
	std::vector<WeatherParticle*>	particles;	//Active particles stay in here :)
	std::vector<WeatherParticle*>	freeList;	//'Spare' particles stay in here...
	Vector4 colour2;
