...
Matrix4 Matrix4::Transpose(Matrix4& src){
	Matrix4 m;

	m.values[1] = src.values[4];
	m.values[2] = src.values[8];
	m.values[4] = src.values[1];
	m.values[6] = src.values[9];
	m.values[8] = src.values[2];
	m.values[9] = src.values[6];

	return m;
}

float Matrix4::RotationMatrixToYaw()
{
	   float tmp = ((atan2(values[8], values[10])) * 180 / PI);

       if(tmp >= 360.0f) tmp = tmp - 360;
