...
void TextMesh::UpdateTextMesh(const std::string &text, const Font &font, const Vector4 &clr)
{
	//Set our Mesh to have the font's texture, containing all of the
	//individual characters we need.
	SetTexture(font.texture);
...
/*
We want to change the data which is already on graphics memory, 
not delete and buffer it again.
*/
void TextMesh::updateText(const std::string &text)
{
	std::string newText = text;

	//Each character has 4 vertices...
	int textLenght = numVertices / 4;

	//Control text lenght, if smaller than the original lenght, append blank spaces
	if (text.length() < (unsigned)textLenght)
	{
		for (unsigned int i = text.length(); i < (unsigned)textLenght; ++i)
		{
			newText.append(" ");
		}
	}
	...
	//(0,0) (top LEFT of the screen intuitively), the text
	//'hangs down' from the top left, and is visible.
	for(unsigned int i = 0; i < (unsigned) textLenght; ++i)
	{
		unsigned int c = (unsigned int)newText[i];
