...
bool	Ray::IntersectsNode(SceneNode &node, Vector3 &intersectPos) {
	...
	float d = (node.GetBoundingRadius()*node.GetBoundingRadius()* 5) - ((relativeDist*relativeDist) - (dot*dot));
...
Vector3 Ray::IntersectsTerrain()	
{
	...
	if (direction.y <0)
	{
			...
			Vector3 dirFinal = direction;
			Vector3 posFinal = position;


			do 
			{	/*posFinal.y = posFinal.y + dirFinal.y;
				posFinal.x = posFinal.x + dirFinal.x;
				posFinal.z = posFinal.z + dirFinal.z;*/
