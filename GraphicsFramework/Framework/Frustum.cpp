...
bool Frustum::CubeInFrustum(Vector3 pos, float size )   
{   
    //given the center of the cube, and half the length.  
	//Then we checking each point in the cube and seeing if it is inside the frustum. 
   
    for(int i = 0; i < 6; ++i)
	{
		if(!planes[i].SphereInPlane(pos,size))
		{
			return false;	//scenenode is outside the plane
		}
	}
	return true;
}  
