...
struct MD5Skeleton {
	...
	MD5Skeleton(const MD5Skeleton& skel)
	{
		joints = new MD5Joint[skel.numJoints];
		memcpy((void*)joints, (void*) skel.joints, sizeof(MD5Joint)* skel.numJoints);
		numJoints = skel.numJoints;
	}
...
public:
	...
	*/
	...
	MD5Skeleton	GetCurrentSkeleton()  {return currentSkeleton;}

	string GetCurrentMesh() { return currentMeshName;}
	//string	GetCurrentAnim() {return currentAnimName;}
	/*
	...
	*/
	MD5Anim*	PlayAnim(std::string name, MD5Anim* currentAnim, unsigned int& currentAnimFrame, float& frameTime);	
	...
	applied MD5Anim.
	*/
	void	UpdateAnim(float msec,  MD5Skeleton* skeleton, MD5Anim* currentAnim, unsigned int& currentAnimFrame, float& frameTime);	
